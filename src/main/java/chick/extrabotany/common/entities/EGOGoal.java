...
public class EGOGoal extends Goal
{
    ...
    public boolean tryAttack()
    {
        ...
        if (!entityEGO.level.isClientSide)
        {
            ...
            {
                case 0 -> ((TrueShadowKatana) ModItems.TRUE_SHADOW_KATANA.get()).attack(entityEGO, target, 3, 0.9D, damageTimes);
                case 1 -> ((TrueTerraBlade) ModItems.TRUE_TERRA_BLADE.get()).attack(entityEGO, target, 3, 0.9D, damageTimes);
                case 2 -> ((TrueThunStarCaller) ModItems.TRUE_THUNSTAR_CALLER.get()).attack(entityEGO, target, 1, 1D, damageTimes);
                case 3 -> ((InfluxWaver) ModItems.INFLUX_WAVER.get()).attack(entityEGO, target, 1, 1.1D, damageTimes);
                case 4 -> ((TrueTerraBlade) ModItems.TRUE_TERRA_BLADE.get()).attack(entityEGO, target, 4, 1.1D, damageTimes);
            }
        }
